...
const PayCard = ({ id, price, percent }: Props) => {
  ...
  const [installmentPeriod, setInstallmentPeriod] = useState<number | undefined>(undefined)
  ...
  const salePriced = () => {
    ...
    return (curPrice - (curPrice * curPercent) / 100).toFixed(2)
  }
  ...
  const pricePerMonth = () => {
    ...
    if (percent) {
      return (Number(salePriced()) / curInstallmentPeriod).toFixed(2)
    }
  ...
  return (
    <div className='flex flex-col gap-4 justify-between shadow-md  p-6  rounded-md min-w-fit h-full'>
        ...
        <div className='flex flex-col gap-1'>
          <div className='flex justify-between '>
            ...
            <div className='flex flex-row content-center gap-1 '>
              ...
              <div className='text-secondary text-base font-medium justify-center'>{percent}%</div>
            </div>
          </div>
      ...
      <Radio.Group onChange={onChange} value={paymentMethod} className='flex flex-col'>
      ...
      <div className='flex flex-col gap-4'>
        <div className='flex flex-row gap-2'>
          {periods.map((month: number) => (
            <label key={month} className='flex flex-col items-center' defaultValue={1}>
              <input
                type='radio'
              ...
              >
                <span className='block text-lg text-center font-light'>{month}</span>
                <span className='text-gray-717171 font-medium text-xs'>Months</span>
              </div>
            </label>
          ))}
        </div>
      ...
      <Button type='primary' className='p-6' onClick={() => alert('on Progress....')}>
        Buy Now
      </Button>
      <Button className='p-6 text-primary border-primary' onClick={handleAddToCart}>
        Add to Cart
      </Button>
    </div>
  )
